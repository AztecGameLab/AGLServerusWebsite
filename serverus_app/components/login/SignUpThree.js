...
class SignUpThree extends React.Component {
  ...
  async redIDCheck(e) {
    ...
    if (e.target.value.length == 9) {
      ...
      isTaken = redIdCheck.redIdTaken;
    }
    this.props.handleRedIDInput(e);
    var that = this;
    if (isTaken && e.target.value != "") {
      that.setState({
    ...
    else if ((e.target.value == "" || e.target.value.length < 9 || e.target.value.length > 9) && !specialMatches) {
      this.setState({
        redIDFirstClick: true,
        redIDWarning: true,
        redIDFilled: false,
                ...
                <Form.Field>
                  ...
                  {!this.state.checked ? <div><label>Red ID</label>
                    <Input inverted placeholder="Red ID" iconPosition='left'>
                      ...
                      {this.state.redIDWarning && this.state.redIDFirstClick && !this.state.redIDTaken && <Label color='red' pointing='left'>Incorrect Red ID</Label>}
                      {this.state.redIDWarning && this.state.redIDTaken && <Label pointing='left' color='red'>Red ID is already used</Label>}
                      {!this.state.redIDWarning && this.state.redIDFirstClick && <Label circular color='green' pointing='left'><Icon name='checkmark' /></Label>}
                    </Input></div> : null}
                </Form.Field>
              </Grid.Column>
            </Grid.Row>
